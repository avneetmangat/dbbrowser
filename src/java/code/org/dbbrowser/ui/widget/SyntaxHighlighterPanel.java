...
//        });
    ...
    /**
     * Returns the selected text or the text in the text area if no text has been selected
     * @return
     */
    public String getText()
    {
        //Get the selected text or if there is no selection, get all the text in the text area
        String sql = this.textPane.getSelectedText();
        if( sql == null )
        {
            sql = this.textPane.getText();
        }

        //Strip the newline characters from the sql
        sql = sql.replaceAll("\r\n", " ");
        sql = sql.replaceAll("\n", " ");
    ...
    private void initDocument() {
        ...
        try {
            document.insertString(document.getLength(), "", getStyle("text"));
